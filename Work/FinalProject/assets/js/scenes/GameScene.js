class GameScene extends Phaser.Scene {
  ...
  addCollisions() {
    ...
    //check for overlaps between the monster and the player
    //this.physics.add.overlap(this.player, this.monsters, this.enemyAttack , null, this)
  }

  enemyAttack(player){
    console.log(player.health);
    if (player.health > 0) {
      player.updateHealth(player.health - .05);
      this.events.emit('updatePlayerHealth', player.id, player.health);
    } else {
      // respawn the player
      this.events.emit('respawnPlayer', player);
      // end game 
      this.events.emit('endGame', player.gold);
    }
  }
